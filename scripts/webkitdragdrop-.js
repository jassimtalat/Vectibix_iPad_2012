...
var webkit_draggable = function(r, ip)
{
	...
	this.touchStart = function(event)
	{
		...
		if (!this.p.disableX) touchX = t.pageX;
		if (!this.p.disableY) touchY = t.pageY;
		...
		p.rx = cp.x;
		p.ry = cp.y;		
		if (!this.p.disableX) p.tx = touchX;
		if (!this.p.disableY) p.ty = touchY;
	...
	this.touchMove = function(event)
	{
		...
		var delX = curX - p.tx;	/* px moved on horisontal axis from touch start */
		var delY = curY - p.ty; /* px moved on vertical axis from touch start */

		var proceed = true;

		var newLeft = p.rx + delX;
		var newTop  = p.ry + delY;

		if (typeof this.p.onMove == 'function')
			proceed = this.p.onMove(event,newLeft,newTop);
		...
		this.lastCurX = curX;
		this.lastCurY = curY;
		if (typeof this.p.onMove == 'function') this.p.onMove(event);
	}
